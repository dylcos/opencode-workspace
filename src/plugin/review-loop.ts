		...
		event: async ({
			...
			if (entry.state === "awaiting_changes") {
				await ctx.client.session.prompt({
					path: { id: sessionID },
					body: {
						parts: [{ type: "text", text: "The reviewer requested changes. Please revise the plan and call plan_save when ready." }],
					},
				})
				...
				await ctx.client.session.prompt({
					path: { id: sessionID },
					body: {
						parts: [{ type: "text", text: "The plan has been revised. Please delegate to the reviewer agent now with the full plan content." }],
					},
				})
			}
		},
